...
# Connects to BitMEX web-socket for streaming real-time data.
# The Market maker still interacts with this as if it were a REST Endpoint, but now it can get
# much more real-time data without heavily polling the API.
#
# The Web-socket offers a bunch of data as raw properties right on the object.
...
class BitMEXWebsocket:
    ...
    def connect(self, endpoint="", symbol="XBTN15", shouldAuth=True):
        """Connect to the web-socket and initialize data stores."""
    ...
    def __connect(self, wsURL):
        """Connect to the web-socket in a thread."""
                        ...
                        if table == 'order':
                            ...
                            if 'cumQty' in updateData and not is_canceled:
                                ...
                                if contExecuted > 0:
                                    ...
                                    self.bot_sendtext('Execution: %s %d Contracts of %s at %.*f' %
    ...
    def __on_open(self, ws):
        self.logger.debug("Web-socket Opened.")
