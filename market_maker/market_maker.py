...
class ExchangeInterface:
    ...
    def bot_sendtext(self, bot_message):
        send_text = 'https://api.telegram.org/bot' + settings.TELEGRAM_BOT_TOKEN + '/sendMessage?chat_id=' + \
                    settings.TELEGRAM_CHAT_ID + '&parse_mode=Markdown&text=' + bot_message
...
class PositionWatcher:
    """ A simple class, set to watch position """
...
class OrderManager:
    def __init__(self):
        ...
        if settings.BASE_URL == "https://testnet.bitmex.com/api/v1/":
            self.bot_sendtext('Starting test net bot using symbol %s' % self.exchange.symbol)
        else:
            self.bot_sendtext('Starting real net bot using symbol %s' % self.exchange.symbol)
    ...
    def get_ticker(self):
        ...
        self.start_position_buy = int(ticker["buy"] + self.instrument['tickSize'] + 0.5)
        self.start_position_sell = int(ticker["sell"] - self.instrument['tickSize'] + 0.5)
        ...
        if settings.MAINTAIN_SPREADS:
            if ticker['buy'] == self.exchange.get_highest_buy()['price']:
                self.start_position_buy = int(ticker["buy"] + 0.5)
            if ticker['sell'] == self.exchange.get_lowest_sell()['price']:
                self.start_position_sell = int(ticker["sell"] + 0.5)
        ...
        if self.start_position_buy * (1.00 + settings.MIN_SPREAD) > self.start_position_sell:
            ...
            self.start_position_buy = int(self.start_position_buy + 0.5)
            self.start_position_sell = int(self.start_position_sell + 0.5)

        # Midpoint, used for simpler order placement.
        self.start_position_mid = int(ticker["mid"] + 0.5)
        logger.info(
    ...
    def get_price_offset(self, index):
        ...
        if settings.MAINTAIN_SPREADS:
            ...
            if index > 0 and start_position < self.start_position_buy:
                start_position = int(self.start_position_sell + 0.5)
            # Same for buys.
            if index < 0 and start_position > self.start_position_sell:
                start_position = int(self.start_position_buy + 0.5)
    ...
    def converge_orders(self, buy_orders, sell_orders):
        ...
        if len(to_amend) > 0:
            for amended_order in reversed(to_amend):
                ...
                amended_order['price'] = int(amended_order['price'] + 0.5)
            ...
            try:
                ...
                error_obj = e.response.json()
                if error_obj['error']['message'] == 'Invalid ordStatus':
                    logger.warn("Amending failed. Waiting for order data to converge and retrying.")
                    sleep(0.5)
                    return self.place_orders()
                else:
                    logger.error("Unknown error on amend: %s. Exiting" % error_obj)
                    sys.exit(1)

        if len(to_create) > 0:
            logger.info("Creating %d orders:" % (len(to_create)))
            for order in reversed(to_create):
                order['price'] = int(order['price'] + 0.5)
    ...
    def exit(self):
        ...
        if settings.BASE_URL == "https://testnet.bitmex.com/api/v1/":
            self.bot_sendtext('Shutting down test bot. Current ContractPosition: %d' % self.running_qty)
        else:
            self.bot_sendtext('Shutting down real bot. Current ContractPosition: %d' % self.running_qty)

        try:
            self.exchange.cancel_all_orders()
            self.exchange.bitmex.exit()
        except errors.AuthenticationError as e:
            logger.info("Was not authenticated; could not cancel orders. %s" % e)
    ...
    def run_loop(self):
        while True:
            ...
            if not self.check_connection():
                logger.error("Real time data connection unexpectedly closed, restarting.")
                self.restart()
