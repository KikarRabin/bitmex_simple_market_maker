...
# Position limits - set to True to activate. Values are in contracts. If you exceed a position limit, the bot will
# log and stop quoting that side. If you are bullish on Bitcoin and don't want to get caught with sell orders when
# BTC skyrockets to the moon without notice, then you may want to only offer buy-side market-making instead of
# two-way market making. The way to do this is to enable CHECK_POSITION_LIMITS and then set MIN_POSITION to -1 so
# that you never hold more than 1 short contract. Set the MAX_POSITION to a number that you feel comfortable with.
# e.g., if your initial buy contract is for 500 contracts and your orders are spaced 1% apart and you want to be able
#  to buy all the way down to BTC dropping 20% in value, then you need 20 * 500 as your MAX_POSITION and no more.
